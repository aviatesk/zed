...
impl LocalLspStore {
    ...
    fn worktree_id_for_language_server(&self, server_id: LanguageServerId) -> Option<WorktreeId> {
    ...
    fn register_buffer_with_language_servers(
        ...
        if let Some(file) = file {
            ...
            initial_snapshot = buffer.text_snapshot();
            worktree_id = state.host_worktree;
            let Some(host_worktree) = self
                .worktree_store
                .read(cx)
                .worktree_for_id(worktree_id, cx)
            else {
...
impl LspStore {
    ...
    pub fn open_untitled_document(
        ...
        if uri.scheme() != "untitled" {
            return Task::ready(Err(anyhow!("expected untitled URI, got {}", uri.as_str())));
        }
        ...
        let Some(local) = self.as_local() else {
            return Task::ready(Err(anyhow!(
                "opening LSP untitled documents requires a local LSP store"
            )));
        };
        if let Some((buffer_id, _)) = local.buffer_uris.iter().find(|(_, state)| state.uri == uri)
            && let Some(buffer) = self.buffer_store.read(cx).get(*buffer_id)
        {
            return Task::ready(Ok(buffer));
        }
        let Some(host_worktree) = preferred_server_id
            .and_then(|server_id| local.worktree_id_for_language_server(server_id))
            .or_else(|| {
                local
                    .worktree_store
                    .read(cx)
            ...
            return Task::ready(Err(anyhow!(
                "opening LSP untitled documents requires a visible worktree"
            )));
        };
        let languages = local.languages.clone();
    ...
    pub(crate) fn open_local_buffer_via_lsp(
        ...
        if abs_path.scheme() == "untitled" {
            return self.open_untitled_document(abs_path, Some(language_server_id), cx);
        }
...
pub struct LanguageServerShowDocumentRequest {
    // Server IDs are local to the LSP store and are not forwarded between peers.
