...
use gpui::{AppContext as _, AsyncApp, Context, Entity, HighlightStyle, Task};
...
fn flatten_document_symbols(
    ...
    for symbol in symbols {
        ...
        // Skip symbols with whitespace-only names that have no detail to display.
        if is_whitespace_name && symbol.detail.as_ref().is_none_or(|d| d.trim().is_empty()) {
        ...
        if let Some(raw_detail) = &symbol.detail {
            ...
            if is_whitespace_name {
                ...
                // Append detail after separator, faded out and searchable.
                let detail_start = text.len() + 1;
                text.push(' ');
                text.push_str(&detail);
                name_ranges.push(detail_start..text.len());
...
mod tests {
    ...
    async fn test_flatten_document_symbols(cx: &mut TestAppContext) {
        ...
        assert_eq!(items[0].name_ranges, vec![7..10, 11..36]);
    ...
    #[gpui::test]
    async fn test_whitespace_name_without_detail_is_skipped(cx: &mut TestAppContext) {
        let buffer = cx.new(|cx| Buffer::local("x = 1\n", cx));
        ...
        // The whitespace-only parent is skipped, but its child is preserved.
