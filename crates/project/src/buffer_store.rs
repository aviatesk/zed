...
use git::{COMMIT_MESSAGE, REBASE_TODO};
use gpui::{
...
impl BufferStore {
    ...
    pub fn open_buffer(
        ...
        if let Some(buffer) = self.get_by_path(&project_path) {
            if should_reload_git_editor_buffer_on_open(buffer.read(cx), cx) {
                let task = match self.loading_buffers.entry(project_path.clone()) {
                    hash_map::Entry::Occupied(entry) => entry.get().clone(),
                    hash_map::Entry::Vacant(entry) => {
                        let task = cx
                            .spawn(async move |this, cx| {
                                let should_reload = buffer.read_with(cx, |buffer, cx| {
                                    should_reload_git_editor_buffer_on_open(buffer, cx)
                                });
...
fn should_reload_git_editor_buffer_on_open(buffer: &Buffer, cx: &App) -> bool {
    if buffer.is_dirty() {
        return false;
    }
    ...
    buffer.file().is_some_and(|file| {
        file.is_local()
            ...
            && matches!(file.file_name(cx), COMMIT_MESSAGE | REBASE_TODO)
    })
}
