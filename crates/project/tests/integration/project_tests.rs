...
#[gpui::test]
async fn test_lsp_show_document_untitled_workspace_edit(cx: &mut gpui::TestAppContext) {
    init_test(cx);
    ...
    let show_document_requests = Arc::new(Mutex::new(Vec::new()));
    project.update(cx, |_, cx| {
        let project = project.clone();
        let show_document_requests = show_document_requests.clone();
        cx.subscribe(&project, move |_, _, event: &Event, _| {
            if let Event::LanguageServerShowDocument(request) = event {
                show_document_requests.lock().push(request.clone());
    ...
    let untitled_uri = Uri::from_str("untitled:/generated.rs").unwrap();
                    ...
                    changes: Some(
                        [(
                            ...
                            vec![lsp::TextEdit {
                                ...
                                new_text: "generated content".into(),
                            }],
                        )]
                        .into_iter()
                        .collect(),
    ...
    let request = {
        let requests = show_document_requests.lock();
        assert_eq!(requests.len(), 1);
        assert_eq!(requests[0].uri, untitled_uri);
        assert!(requests[0].server_id.is_some());
        requests[0].clone()
    };
    let buffer = project
        .update(cx, |project, cx| {
    ...
    let open = fake_server
        .receive_notification::<lsp::notification::DidOpenTextDocument>()
        .await;
    assert_eq!(open.text_document.uri, untitled_uri);
    assert_eq!(open.text_document.text, "generated content");
...
#[gpui::test]
async fn test_open_lsp_untitled_document_without_worktree(cx: &mut gpui::TestAppContext) {
    init_test(cx);
    let project = Project::test(FakeFs::new(cx.executor()), Vec::<&Path>::new(), cx).await;
    let result = project
        .update(cx, |project, cx| {
            project.open_lsp_untitled_document("untitled:/generated.rs".parse().unwrap(), None, cx)
        })
        .await;
