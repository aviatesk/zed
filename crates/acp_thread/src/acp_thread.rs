...
impl ToolCall {
    fn from_acp(
        ...
        let tool_name = tool_call
            ...
            .or_else(|| tool_name_from_meta_or_title(&tool_call.meta, &tool_call.title));
        let title =
    ...
    fn update_fields(
        ...
        if let Some(tool_name) = name.map(SharedString::from) {
            ...
            // Legacy metadata and bridge titles only fill a missing name so they
            // cannot replace a first-class name received earlier.
            self.tool_name = tool_name_from_meta_or_title(&meta, title.as_deref().unwrap_or(""));
